...
@section('title', 'Update Vehicle Brand')

@section('content')
    <section class="admin-page-header">
        <div>
            <h1>Update Vehicle Brand</h1>
            <p>Edit the selected vehicle brand name.</p>
        </div>

        <a href="{{ route('admin.categories.vehicle-brand') }}" class="admin-badge" style="text-decoration: none;">Back to Vehicle Brands</a>
    </section>
    ...
    <form method="POST" action="{{ route('admin.categories.vehicle-brand.update', $vehicleBrand) }}" style="width: 100%; max-width: 520px; box-sizing: border-box; padding: 24px; border-radius: 16px; background: #fff; box-shadow: 0 14px 32px rgba(36, 103, 255, 0.1);">
        @csrf
        @method('PUT')
